...
12. **Multi-Curve Coordinate Graph Visual Reasoning & Scientific Unit Anchor (Giải mã biểu đồ tọa độ đa đường & Đơn vị đo lường khoa học):**
    * Trích xuất và suy luận trực tiếp trên các hình vẽ biểu đồ khoa học 2D nhiều đường biểu diễn với kiểu nét vẽ khác nhau (nét liền, nét đứt, nét chấm gạch) kèm theo hệ trục tọa độ chia vạch chi tiết và đơn vị đo lường chuyên biệt (*phần nghìn ‰, mmol/L, ppm*). Câu hỏi yêu cầu đọc giá trị cực trị/đoạn tối ưu của một đường cụ thể. Kỹ thuật này triệt tiêu hoàn toàn OCR thông thường và đòi hỏi năng lực đọc hiểu biểu đồ thị giác (PlotQA / Visual Chart Reasoning).
13. **Multi-Entity Pursuit Color Binding & No-Hands Victory Gesture (Ràng buộc đa thực thể bám đuổi & Tư thế ăn mừng rời tay lái):**
    * Khai thác góc quay từ trên cao nhìn xuống (Top-Down Overhead POV) của phân cảnh rút đích tốc độ cao, đặt ràng buộc kép giữa **tư thế chuyển động bất thường** (*buông cả 2 tay khỏi ghi-đông giang cao trong khi xe trôi qua vạch đích*) của người dẫn đầu và **màu sắc áo đấu của 2 đối thủ bám đuổi sát nút phía sau** (*áo vàng và áo cam*). Kỹ thuật này triệt tiêu hoàn toàn tìm kiếm từ khóa thể thao chung chung và buộc mô hình phải phân giải không gian đa thực thể (Multi-Entity Spatial Alignment) và nhận diện tư thế động học (Kinematic Pose Recognition).
14. **Dual-Layer Nested Visual Grounding: Presenter PIP & Slide Stock Photo Binding (Ràng buộc thị giác lồng ghép đa tầng Người thuyết trình - Ảnh minh họa slide):**
    * Cấu trúc câu hỏi ràng buộc đồng thời hai vùng thị giác độc lập trong cùng một khung hình video giảng dạy/thuyết trình: (1) **Tầng người thuyết trình** (Presenter PIP góc màn hình: giới tính, kính mắt, đặc điểm trang phục áo sơ mi sọc vai, cử chỉ 2 tay) và (2) **Tầng ảnh minh họa trên slide** (Slide Stock Photo: tư thế ngồi khoanh chân trên sofa xám, tay cầm cốc nước, nhìn vào laptop trên đùi). Kỹ thuật này triệt tiêu các mô hình chỉ nhìn vào slide hoặc chỉ nhìn vào người giảng, ép mô hình phải phân tách và liên kết nhiều vùng quan tâm (Multi-Region Spatial Disambiguation).
15. **Multi-Style Graphical Metaphor Binding & Lecture Concept Anonymization (Ẩn dụ đồ họa đa phong cách & Trừu tượng hóa khái niệm bài giảng):**
    * Triệt tiêu toàn bộ thuật ngữ chuyên môn của bài giảng (*kinh tế học, cạnh tranh thị trường, quy luật giá trị*), thay vào đó neo chặt chẽ vào **tổ hợp các hình vẽ ẩn dụ đồ họa mang phong cách thị giác đối lập** cùng xuất hiện trên một trang slide (ví dụ: *mô hình 3D người nhân tạo bóng bẩy màu trắng vây quanh một người màu đỏ ở giữa $\oplus$ tranh hoạt hình phẳng 2D hai người đàn ông kéo co đối đầu nhau với sợi dây thừng*). Kỹ thuật này đánh bại toàn bộ các mô hình NLP/Text-Retrieval và bắt buộc hệ thống giải phải có năng lực phân tích ngữ nghĩa đồ họa và nhận diện phong cách hình ảnh đa dạng (Multi-Style Clipart & Vector Semantic Parsing).
...
9. **Negative OCR Set Subtraction & Occluded Physical Pillar Inventory (Phép trừ tập hợp ký tự OCR & Kiểm kê cột trụ bị che khuất):**
   * Đặt câu hỏi kiểm kê một dãy số thứ tự liên tục ($1 \dots N$) được dán trên các cột trụ vật lý trong phân cảnh biểu diễn, nhưng yêu cầu trích xuất **các con số bị khuất / không nhìn thấy từ góc máy của camera** ($\{1..N\} \setminus \text{Visible}$). Kỹ thuật này triệt tiêu các mô hình OCR thông thường chỉ biết trích xuất những gì nhìn thấy (Positive Recognition) và bắt buộc hệ thống giải phải có khả năng suy luận logic tập hợp phủ định và hiểu biết về không gian 3D (Set Difference & 3D Spatial Occlusion Reasoning).
10. **Multi-Prop Culinary Background Spatial Inventory & Multi-Meat Component Gram OCR (Kiểm kê bố cục đạo cụ ẩm thực hậu cảnh & Trích xuất khối lượng gram từng phần thịt):**
   * Khóa chặt phân cảnh bảng thành phần bằng một danh mục $\ge 7$ đạo cụ/nguyên liệu được sắp đặt tinh vi ở hậu cảnh (dĩa thịt, bó lá tươi, gói gia vị, hũ thủy tinh nước cốt dừa, mộc nhĩ khô, sả cây, ớt hiểm), sau đó đặt câu hỏi trích xuất **chính xác từng định lượng gram (g) và tên cụ thể của từng loại thịt** xuất hiện trong bảng danh mục (*350g thịt bò mềm, 100g thịt nạc dăm* thay vì chỉ đọc tổng số hoặc tên chung chung). Kỹ thuật này triệt tiêu hoàn toàn khả năng đoán mò và buộc hệ thống giải phải đọc hiểu cấu trúc danh sách chi tiết (Structured Recipe Key-Value OCR).
...
8. **Micro-Fibrous Texture Anonymization & Animal Biological Origin QA Bridge (Ẩn danh kết cấu sợi nhỏ & Truy vết nguồn gốc động vật của topping):**
   * Miêu tả đặc điểm macro cực đại của một loại đồ ăn kèm đã qua chế biến biến tính hình thái (*topping màu cam kết cấu sợi nhỏ trong chén màu đen kế bên tô cháo đã hoàn thiện*), sau đó đặt câu hỏi truy ngược về nguồn gốc sinh học của nguyên liệu (*'thịt của con gì?'* thay vì hỏi tên món ăn). Kỹ thuật này triệt tiêu hoàn toàn OCR và NLP thông thường, buộc hệ thống giải phải định vị được phân đoạn trình bày thành phẩm rồi truy ngược về công đoạn sơ chế/bảng nguyên liệu ban đầu để suy ra loài động vật (*con cá hồi*).
9. **Algebraic Entity Variable Substitution 'X' & Sequential Interaction QA (Biến số đại số ẩn danh thực thể 'X' & Chuỗi thao tác tương tác số lượng):**
   * Thay thế toàn bộ danh từ định danh thực thể bí mật bằng ký hiệu biến số đại số (*'4 con X'*, *'2 con X'*, *'X là nguyên liệu cho món ăn'*), kết hợp chuỗi thao tác đếm tương tác vật lý tuần tự (*đặt lên dĩa trắng 4 con $\rightarrow$ cầm lên 2 con $\rightarrow$ bắt đầu đối thoại đoán tên món*), sau đó đặt câu hỏi định danh loài của biến số $X$ (*'Hỏi X là con gì?'*). Kỹ thuật này triệt tiêu hoàn toàn khả năng search từ khóa bề mặt của text embedding, bắt buộc mô hình phải phân tích ngữ cảnh hành vi kết hợp nhận diện hình thái sinh học đặc thù (*con hải sâm*) để giải mã biến số $X$.
